...
if($act == 't'){
	...
	if($id <= 0){
		$stmt = $con->prepare("INSERT INTO hutang(tanggal, catatan, kreditur, jumlah, user, status) VALUES(?, ?, ?, ?, ?, ?)");
		$stmt->bind_param("sssdis", $tanggal, $catatan, $kreditur, $jumlah, $user, $status);
		$stmt->execute();
		$stmt->close();
